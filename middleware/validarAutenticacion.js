...
const validarAutenticacion = async (req, res, next) => {
    ...
    if (req.headers.authorization && req.headers.authorization.startsWith("Bearer")) {
        try {
            ...
            req.usuario = await Usuario.findById(decode.id).select("-claveAcceso -estadoUsuario -direccionUsuario -correoUsuario -celularUsuario -idEspecialidad -idRol -createdAt -updatedAt -__v");
